...
public class SerializationTests
{
    ...
    {
        // Arrange
        VsaResult<int> errorOr = 42;
        var options = CreateOptions();

        // Act
        var json = JsonSerializer.Serialize(errorOr, options);
        var deserialized = JsonSerializer.Deserialize<VsaResult<int>>(json, options);
    ...
    {
        // Arrange
        VsaResult<int> errorOr = Error.NotFound("Item.NotFound", "Item was not found");
        var options = CreateOptions();
    ...
    {
        // Arrange
        VsaResult<int> errorOr = new List<Error>
        {
    ...
    {
        // Arrange
        VsaResult<TestPerson> errorOr = new TestPerson { Name = "John", Age = 30 };
        var options = CreateOptions();

        // Act
        var json = JsonSerializer.Serialize(errorOr, options);
        var deserialized = JsonSerializer.Deserialize<VsaResult<TestPerson>>(json, options);
